...
const GBA = struct {
    const MEM_IO = @intToPtr(*volatile u32, 0x04000000);
    const VRAM = @intToPtr([*]volatile u16, 0x06000000);
    const REG_DISPCNT = @intToPtr(*volatile u32, @ptrToInt(MEM_IO) + 0x0000);
